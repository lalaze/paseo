...
- **Text fragments retain their content block identity when the provider supplies one.** Pi can emit the first answer token before the last thinking fragment from the same SSE packet. Preserve its message-scoped `contentIndex` as `blockId` through coalescing, projection, live rendering, and replica storage. Updates append to that block at its original position; canonical catch-up replaces its complete text. Never infer block identity from the text itself or merge different identified blocks merely because they are adjacent.
- **The leading flush adds a canonical row, and that is fine.** A burst's first chunk lands as its own timeline row. `timeline-projection.ts` merges identified blocks by identity and joins contiguous same-turn chunks without block IDs. Clients read the projected timeline, so history is unaffected. Tests that assert on raw rows have to account for the extra row; tests that assert on what a client sees do not.
