...
export interface ConversationGateway {
  ...
  conversationHistory(
    agentId: string,
    stop?: (items: AgentTimelineItem[]) => boolean,
...
const messageId = (item: AgentTimelineItem) =>
  item.type === "user_message" ? (item.clientMessageId ?? item.messageId) : undefined;
const normalize = (text: string) => text.trim().replace(/[。！!\s]+$/u, "");
const approvals = {
  approve_plan: ["批准方案", "确认方案", "方案通过"],
  accept_final: ["验收通过", "确认验收", "验收通过，完成任务"],
  reject_final: ["不采纳成果"],
};
...
export function validateChatApproval(
  items: AgentTimelineItem[],
  input: { messageId: string; noticeId: string; action: keyof typeof approvals },
) {
  ...
  if (
    ...
    !approvals[input.action].includes(normalize(user.text))
  )
...
export class Conversations {
  ...
  summary(id: string): ConversationSummary {
    ...
    return {
      ...
      state: c.state,
      error: c.error,
      title: run?.goal ?? c.initialGoal ?? "新的协作对话",
      run,
    ...
    const id = await this.locked("create", async () => {
      ...
      if (!c && !input.fresh && !input.goal) {
        ...
        c =
          matches.find((candidateConversation) => {
            const run = candidateConversation.runId
              ? this.store.get(candidateConversation.runId)
              : undefined;
      ...
      if (!c) {
        ...
        c = {
          ...
          cwd: await this.gateway.workspaceDirectory(input.workspaceId),
          settings,
          createdAt: Date.now(),
          state: "creating",
    ...
    const id = await this.locked("create", async () => {
      ...
      if (!c) {
        ...
        if (settings.profiles.length >= 30)
          throw new Error("协作配置已满，请先移除一个未使用的 AI 配置再接管当前对话");
      ...
      if (
        !c.takeover.messages.some((m) => m.id === commandId) &&
        (input.goal?.trim() || !c.takeover.messages.length)
      ) {
        c.takeover.messages.push({
          id: commandId,
          text: input.goal?.trim() || ENABLE_COLLABORATION_MESSAGE,
          automatic: !input.goal?.trim(),
          state: "pending",
          createdAt: Date.now(),
        });
        this.store.saveConversation(c);
  ...
  private async recoverReceipt(c: Conversation) {
    ...
    if (recovered) {
      ...
      for (const entry of Object.values(c.receipts))
        if (entry.action === "start" && entry.state === "pending") {
          entry.state = "done";
          entry.value = { runId: recovered.id };
        }
      this.store.saveConversation(c);
    }
  }
  private async deliverTakeover(c: Conversation) {
    for (const message of c.takeover!.messages.filter((m) => m.state !== "sent")) {
      ...
      if (message.state === "sending")
        throw new Error("无法确认接管消息是否送达，请重新同步到主对话后重试");
  ...
  private async ensure(c: Conversation) {
    try {
      ...
      if (!c.runId) {
        await this.recoverReceipt(c);
      }
      await this.ensureSession(c);

      if (c.state !== "ready" || c.error !== undefined) {
        c.state = "ready";
        c.error = undefined;
        this.store.saveConversation(c);
      }
      ...
      if (c.takeover) {
        if (!(await this.deliverTakeover(c))) return;
      }
      if (!c.initialDelivered && c.initialGoal && !c.runId) {
        await this.deliverInitialGoal(c);
      }
    } catch (error) {
  ...
  private async user(
    c: Conversation,
    id?: string,
    need: (items: AgentTimelineItem[]) => boolean = () => true,
  ) {
    if (!c.agentId) throw new Error("主会话尚未建立");
    const automatic = new Set([
      ...(c.takeover?.messages.filter((m) => m.automatic).map((m) => m.id) ?? []),
      ...c.notices.map((n) => n.id),
      ...(c.runId ? this.store.get(c.runId).operations.map((o) => o.id) : []),
    ]);
    const firstCommand = c.takeover?.messages[0]?.id;
    const candidate = (i: AgentTimelineItem) =>
      i.type === "user_message" && !!messageId(i) && !automatic.has(messageId(i)!);
    ...
    const items = await this.gateway.conversationHistory(
      c.agentId,
      (history) =>
        need(history) &&
        (delivered
          ? history.some((i) => candidate(i) || messageId(i) === firstCommand)
          : history.some((i) => messageId(i) === firstCommand)),
    );
    const boundary = firstCommand ? items.findIndex((i) => messageId(i) === firstCommand) : 0;
    const eligible = boundary < 0 ? [] : items.slice(boundary);
    const latest = eligible.findLast(candidate);
    if (id && (!latest || messageId(latest) !== id))
      throw new Error("请基于主对话最新的真实用户消息操作");
    return {
      items,
      latest:
        latest?.type === "user_message" ? { id: messageId(latest)!, text: latest.text } : undefined,
    };
  }
  ...
  async start(id: string, input: { sourceMessageId: string; goal: string }) {
    return this.locked(id, async () => {
      ...
      c.runId = runId;
      c.receipts[key] = { action: "start", state: "done", value: { runId } };
      this.store.saveConversation(c);
      return { runId };
    });
  }
  async submit(id: string, operationId: string, payload: unknown) {
    return this.locked(id, async () => {
      ...
      if (
        !run ||
        !op ||
        op.agentId !== c.agentId ||
        op.id !== operationId ||
        !["plan", "final", "review"].includes(op.kind)
      )
        throw new Error("主 Agent 无权提交此步骤");
  ...
  async control(
    ...
    return this.locked(id, async () => {
      ...
      const { items } = await this.user(
        c,
        input.sourceMessageId,
        (history) => !anchor || history.some((i) => messageId(i) === anchor),
      );
      ...
      if (["approve_plan", "accept_final", "reject_final"].includes(input.action)) {
        ...
        if (
          !current ||
          !c.confirmation ||
          current.key !== c.confirmation.key ||
          input.confirmationKey !== current.key ||
          (input.action === "approve_plan") !== (current.kind === "plan")
        )
          throw new Error("确认请求已过期，请先读取当前方案或成果");
        validateChatApproval(items, {
          messageId: input.sourceMessageId,
          noticeId: c.confirmation.noticeId,
          action: input.action as keyof typeof approvals,
        });
      }
      if (["revise", "cancel"].includes(input.action))
        for (const entry of Object.values(c.receipts))
          if (entry.action === "revise" && entry.state === "pending") entry.state = "done";
      c.receipts[receipt] = { action: input.action, state: "pending", goal: input.goal };
      this.store.saveConversation(c);
      if (input.action === "revise" && ["running", "waiting_permission"].includes(run.control)) {
        await this.engine.control(run.id, "pause");
        run = this.store.get(run.id);
      }
      // Retrying a revise continues the same stop-and-reconcile transaction.
      await this.engine.control(
        run.id,
        input.action,
        input.goal,
        {
      ...
      c.receipts[receipt] = { action: input.action, state: "done" };
      this.store.saveConversation(c);
      return { ok: true };
    });
  }
  async migrate() {
    return this.locked("create", async () => {
      for (const run of this.store.legacyRuns()) {
        ...
        let c = this.store
          .conversations()
          .find((candidateConversation) => candidateConversation.id === id);
        if (!c) {
          const workspaceId = run.workspaceId ?? "";
          const saved = this.store.settings(),
            lead = saved?.profiles.find((p) => p.id === saved.directorProfileId);
          const settings = SettingsSchema.parse({
            ...run.settings,
            profiles: run.settings.profiles.map((p) =>
              p.id === run.settings.directorProfileId && lead ? { ...lead, id: p.id } : p,
            ),
          });
          c = {
            id,
            ...
            legacyAgentId: run.directorAgentId,
            notices: [],
            receipts: {},
          };
          this.store.saveConversation(c);
  ...
  async resync(id: string) {
    return this.locked(id, async () => {
      ...
      if (c.takeover) {
        if (
          !c.agentId ||
          (await this.gateway.inspect(c.agentId, "recover-main", Date.now())).status === "missing"
        )
          throw new Error("原对话已不可用，无法原地接管；请从历史恢复此对话");
        c.linksReady = false;
        for (const message of c.takeover.messages)
          if (message.state === "sending") {
            const state = await this.gateway.inspect(c.agentId, message.id, message.createdAt);
      ...
      if (c.takeover && !c.takeover.messages.some((m) => m.state !== "sent")) {
        c.takeover.messages.push({
          id: `chat-command:resync:${c.id}:${c.takeover.messages.length}`,
          text: "用户已在当前对话启用协作。请重新查询状态，确认工具可用；这不是新的实施任务。",
          automatic: true,
          state: "pending",
          createdAt: Date.now(),
        });
  ...
  private async checkHandshake(c: Conversation) {
    if (c.agentId && !c.toolsConnectedAt && !c.runId) {
      const state = await this.gateway.inspect(c.agentId, "tool-handshake", Date.now());
      const { latest } = await this.user(c);
      if (
        (latest || c.takeover?.messages.some((m) => m.state === "sent")) &&
        ["idle", "error"].includes(state.status)
      ) {
        c.error =
          "主 Agent 尚未成功连接内置协作工具。请检查供应商的 Paseo 工具设置及当前会话错误，然后重新同步。";
        this.store.saveConversation(c);
      }
    }
  }
  private async recoverRevisions(c: Conversation) {
  ...
  private async updateNotice(c: Conversation) {
    const run = this.store.get(c.runId!),
    ...
    const key = digest(
      JSON.stringify([
        ...
        run.control,
        run.planVersion,
        run.tasks.map((candidateEntry) => [
          candidateEntry.spec.id,
          candidateEntry.status,
          candidateEntry.result?.summary,
        ]),
        confirmation?.key,
      ]),
    );
  ...
  private async deliverNotice(c: Conversation) {
    for (const notice of c.notices.filter((n) => n.state !== "sent")) {
      ...
      if (notice.state === "sending") {
        c.error =
          "无法确认上次通知是否送达；可在主对话询问进度。为避免重复发送，已保留待核对记录。";
        this.store.saveConversation(c);
        break;
      }
      if (["missing", "error"].includes(state.status)) {
        c.error =
          state.status === "missing"
            ? "主会话已不可用，请重新同步以创建承接会话，任务进度会保留。"
            : "主 Agent 会话出错，请检查原生会话错误并重试。";
        this.store.saveConversation(c);
        break;
      }
      ...
      await this.locked(id, async () => {
        ...
        try {
          if (
            ...
            !c.runId ||
            c.state !== "ready" ||
            (!c.initialDelivered && c.initialGoal && !c.runId)
          )
            await this.ensure(c);
          if (c.takeover?.messages.some((m) => m.state !== "sent")) return;
          await this.checkHandshake(c);

          if (!c.agentId || !c.runId) return;
          await this.recoverRevisions(c);

          const op = await this.updateNotice(c);
