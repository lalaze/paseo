...
import { awaitingAcceptance, summarize, type Run } from "@getpaseo/protocol/collaboration/schema";
...
import { harness, plan, result, review } from "./test-utils/harness.js";
...
test("AI final approval waits durably for the user, never resumes itself or consumes the waiting budget", async (t) => {
  ...
  assert.equal(before.phase, "awaiting_acceptance");
  assert.equal(before.control, "paused");
  assert.equal(before.userAcceptance, undefined);
  assert.equal(before.activeOperationId, undefined);
  assert.equal(before.operations.at(-1)!.state, "done");
  await h.restart();
  h.elapse(before.settings.runTimeoutMs * 2);
  for (let i = 0; i < 3; i++) await h.engine.tick();
  assert.equal(h.run().revision, before.revision);
  assert.equal(h.agents.sent.length, sent);
  for (const action of ["resume", "retry", "approve_plan", "revise"] as const)
    await assert.rejects(h.engine.control(h.id, action, "变更目标"), /验收成果或提交修改意见/);
  await h.engine.control(h.id, "accept_final", undefined, input(h.run()));
  assert.equal(h.run().phase, "completed");
  assert.equal(h.run().userAcceptance!.decision, "approved");
...
test("user feedback preserves goal, evidence and team, gates the revised plan, and requires another final acceptance", async (t) => {
  const h = await harness({ requirePlanApproval: true, maxAttempts: 3 });
  t.onTestFinished(() => h.cleanup());
  await finishRound(h);
  const original = h.run(),
    worker = original.tasks[0].agentId;
  ...
  assert.equal(h.run().goal, original.goal);
  assert.equal(h.run().cwd, original.cwd);
  assert.equal(h.run().branch, original.branch);
  ...
  await h.restart();
  const planning = await h.until("plan"),
    draft = context(planning.prompt);
  assert.equal(planning.agentId, original.directorAgentId);
  assert.deepEqual(draft.previousPlan, original.plan);
  assert.equal(draft.previousTaskResults[0].agentId, worker);
  ...
  assert.equal(h.run().operations.length, 6);
  // Same artifact in a newer round is still a different acceptance decision.
  await assert.rejects(
    h.engine.control(h.id, "accept_final", undefined, input(original)),
    /版本已变化/,
  );
  await h.engine.control(h.id, "accept_final", undefined, input(h.run()));
  assert.equal(h.run().phase, "completed");
...
test("legacy AI-completed tasks allow acceptance or changes without recreating the workspace", async (t) => {
  ...
  h.store.save(old);
  await h.restart();
  assert.equal(awaitingAcceptance(h.run()), true);
  assert.equal(summarize(h.run()).phase, "awaiting_acceptance");
  const original = h.run();
...
test("user can reject a final result without restarting agents or removing files", async (t) => {
  ...
  const old = h.run(),
    sent = h.agents.sent.length;
  h.repository.capture = async () => {
    throw new Error("不应读取或修改仓库");
  };
  await h.engine.control(h.id, "reject_final", undefined, input(old));
  await h.restart();
  await h.engine.tick();
  assert.equal(h.run().control, "canceled");
  assert.equal(h.run().userAcceptance!.decision, "rejected");
  assert.deepEqual(h.run().finalEvidence, old.finalEvidence);
  assert.equal(h.run().cwd, old.cwd);
  assert.equal(h.agents.sent.length, sent);
  assert.equal(h.agents.stopped.length, 0);
  await assert.rejects(
    h.engine.control(h.id, "accept_final", undefined, input(h.run())),
    /没有可验收/,
  );
});
...
test("final controls reject absent/stale evidence, empty feedback and acceptance of changed code", async (t) => {
  ...
  await assert.rejects(
    h.engine.control(h.id, "accept_final", undefined, { ...input(before), artifactId: "old" }),
    /版本已变化/,
  );
  await assert.rejects(
    h.engine.control(h.id, "request_changes", undefined, { ...input(before), feedback: "   " }),
    /填写修改意见/,
  );
  h.repository.version = "artifact-v2";
  await assert.rejects(
    h.engine.control(h.id, "accept_final", undefined, input(before)),
    /代码已在最终审核后发生变化/,
  );
  assert.equal(h.run().revision, before.revision);
  assert.equal(h.run().userAcceptance, undefined);
  h.repository.assertBranch = async () => {
    throw new Error("请切回成果分支");
  };
  await assert.rejects(
    h.engine.control(h.id, "request_changes", undefined, {
      ...input(before),
      feedback: "修改说明",
    }),
    /切回成果分支/,
...
test("blocked final reviews cannot be approved by the user as a shortcut", async (t) => {
  ...
  await h.complete(result);
  await h.until("final");
  await h.complete(review(true, "blocked"));
...
test("duplicate user acceptance is applied once", async (t) => {
  ...
  const results = await Promise.allSettled(
    [1, 2].map(() => h.engine.control(h.id, "accept_final", undefined, decision)),
  );
...
test("reopening a finished result cannot overlap another task in its directory", async (t) => {
  ...
  assert.equal(h.run().phase, "completed");
  assert.equal(h.run().changeRequests, undefined);
});
...
test("pending final acceptance restores in its workspace and holds the checkout until the user decides", async (t) => {
  ...
  const run = { ...h.run(), workspaceId: "workspace", cwd: h.directory };
  h.store.save(run);
  await h.restart();
