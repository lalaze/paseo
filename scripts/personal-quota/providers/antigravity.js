...
export function quotaWindows(payload) {
  ...
  const data =
    payload?.userStatus?.cascadeModelConfigData ?? payload?.cascadeModelConfigData ?? payload;
  for (const [i, model] of array(data?.clientModelConfigs).entries()) {
    if (!model?.quotaInfo) continue;
    add(
      `agy_model_${model.modelOrAlias?.model ?? i}`,
      `${model.label ?? `Model ${i + 1}`} · Quota`,
...
export class AntigravityQuotaProvider {
  ...
  async fetchFresh() {
    try {
      ...
      this.logger.debug(
        "Antigravity local quota unavailable; check agy login and local API readiness",
      );
