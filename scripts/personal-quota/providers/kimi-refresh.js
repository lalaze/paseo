...
async function binaryPath() {
  ...
  try {
    const c = JSON.parse(
      await fs.readFile(
        join(process.env.PASEO_HOME || join(homedir(), ".paseo"), "config.json"),
        "utf8",
      ),
    );
    configured = c.agents?.providers?.kimi?.command?.[0];
  } catch {
    /* Standard install fallback. */
  }
  const explicit = process.env.PASEO_KIMI_BIN;
  const fromPath = (process.env.PATH || "")
    .split(delimiter)
    .filter(Boolean)
    .map((dir) => join(dir, "kimi"));
  const candidates = [
    explicit,
...
async function descendantPids(pid) {
  const kids = (await run("/usr/bin/pgrep", ["-P", String(pid)]))
    .trim()
    .split(/\s+/)
    .map(Number)
    .filter((n) => n > 0);
...
async function childPorts(pid) {
  if (process.platform === "darwin") {
    ...
    for (const p of [pid, ...(await descendantPids(pid))]) {
      for (const port of portsFromLsof(
        await run("/usr/sbin/lsof", ["-nP", "-a", "-p", String(p), "-iTCP", "-sTCP:LISTEN", "-Fn"]),
      ))
        ports.add(port);
    }
    return [...ports];
  ...
  for (const table of ["tcp", "tcp6"]) {
    let rows;
    try {
      rows = await fs.readFile(`${base}/net/${table}`, "utf8");
    } catch {
      continue;
    }
    for (const row of rows.trim().split("\n").slice(1)) {
      const parts = row.trim().split(/\s+/);
      if (parts[3] === "0A" && inodes.has(parts[9]))
        ports.add(parseInt(parts[1].split(":")[1], 16));
    }
  }
  return [...ports];
}
...
export function queryLocalUsage(
  port,
  token,
  timeoutMs,
  hostname = "127.0.0.1",
  path = "/api/v1/oauth/usage",
) {
  return new Promise((resolve) => {
    let timer;
    let settled = false;
    ...
    const req = http.get(
      { hostname, family: hostname.includes(":") ? 6 : 4, port, path, agent: false, headers },
      (res) => {
        const chunks = [];
        let bytes = 0;
        res.on("data", (chunk) => {
        ...
        res.on("end", () => {
          try {
            ...
            const kind = body?.data?.kind;
              ...
              res.statusCode === 200 && (kind === "ok" || kind === "error" || kind === undefined),
            );
          } catch {
...
function credentialsAreFresh(raw, nowMs) {
  try {
    return !needsKimiRefresh(JSON.parse(raw), nowMs);
  } catch {
    return false;
  }
}
...
export async function renewWithKimi(credentialsPath, dependencies = {}) {
  if (process.platform !== "linux" && process.platform !== "darwin")
    throw new Error("Linux or macOS required");
  ...
  const readToken =
    dependencies.readToken ?? (() => fs.readFile(join(kimiHome, "server.token"), "utf8"));
  const readCredentials =
    dependencies.readCredentials ?? (() => fs.readFile(credentialsPath, "utf8"));
  ...
  const child = start(bin, ["web", "--no-open", "--host", "127.0.0.1"], {
    cwd: kimiHome,
    stdio: dependencies.stdio ?? "ignore",
    detached: true,
    env: {
      ...process.env,
  ...
  const onExit = (code, signal) => {
    exited = true;
    trace(`kimi web exited code=${code ?? "null"} signal=${signal ?? "null"}`);
  };
  child.once("exit", onExit);
  child.once("error", (error) => {
    exited = true;
    trace(`kimi web spawn error ${error.message}`);
  });
  if (child.stdout)
  ...
  const kill = (signal) => {
    if (!exited && child.pid) {
      ...
      try {
        (dependencies.killGroup ?? process.kill)(-child.pid, signal);
      } catch {
        /* Already gone. */
      }
      try {
  ...
  try {
    ...
    while (!exited && now() < deadline) {
      ...
      let token = "";
      try {
        ports = [...new Set([...ports, ...(await portsFor(child.pid))])];
      } catch {
        /* Starting up. */
      }
      try {
        ...
        /* Token file appears after listen. */
      }
    ...
    throw new Error(
      exited
        ? "Kimi credential renewal unavailable; kimi web exited"
        : "Kimi credential renewal unavailable",
    );
  } finally {
    cleanup();
