...
export async function resolveBinaries() {
  ...
  try {
    config = JSON.parse(
      await fs.readFile(
        join(process.env.PASEO_HOME || join(home, ".paseo"), "config.json"),
        "utf8",
      ),
    );
  } catch {
    /* Use standard installation below. */
  }
  const acp = config.agents?.providers?.["antigravity-acp"]?.env?.AGY_BIN;
  const hub =
    process.env.AGY_HUB_BIN || config.agents?.providers?.["antigravity-hub"]?.env?.AGY_HUB_BIN;
...
export async function listeningPorts(pid) {
  if (process.platform === "darwin") {
    ...
    for (const p of [pid, ...(await descendantPids(pid))]) {
      for (const port of parseLsofListenPorts(
  ...
  for (const table of ["tcp", "tcp6"]) {
    let data;
    try {
      data = await fs.readFile(`${base}/net/${table}`, "utf8");
    } catch {
      continue;
    }
...
async function probe(pid, csrf, parse, deadline, methods = METHODS) {
  let ports;
  try {
    ports = await listeningPorts(pid);
  } catch {
    return null;
  }
  if (!csrf)
    csrf = await csrfFromOwnedPorts(ports, Math.min(800, Math.max(1, deadline - Date.now())));
...
async function processExeMatches(pid, bin) {
  ...
  try {
    ...
    try {
      if ((await fs.realpath(link)) === bin) return true;
    } catch {
      /* Replaced or deleted. */
    }
  } catch {
    /* Not a linux proc exe link. */
  }
  return false;
}
...
async function reuseBinary(bin, parse, deadline) {
  if (process.platform === "darwin") {
    const pids = parseLsofTxtPids(
      await run("/usr/sbin/lsof", ["-nP", "-u", String(process.getuid()), "-Fpf", bin]),
    );
    for (const pid of pids) {
      if (Date.now() >= deadline) break;
      const csrf = csrfFromCommand(
        await run("/bin/ps", ["-p", String(pid), "-www", "-o", "args="]),
      );
...
export async function readLocalQuota(parse, { trace = () => {} } = {}) {
  ...
  let exited = false;
  const exitEvent = proc.onExit((event) => {
    exited = true;
    trace(`spawn exited code=${event?.exitCode ?? "unknown"}`);
  });
  ...
  try {
    ...
    trace(
      exited
        ? "agy exited before quota API was ready"
        : "no quota windows from process-owned ports",
    );
    return null;
  } finally {
    cleanup();
